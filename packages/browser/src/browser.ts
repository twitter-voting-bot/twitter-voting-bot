...
import { sleep, logger } from '@twitter-voting-bot/utils';
...
export default function browser() {
  ...
  async function googleAuth(url: string) {
    const logger = logging.child('googleAuth');
    logger.debug(`Starting the browser for google auth, will auth on '${url}'`);
    ...
    logger.debug(`Typing in the e-mail to the input and pressing enter`);
    ...
    logger.debug(`Typing in the password to the input and pressing enter`);
    ...
    if (page.url().includes('oauth/warning')) {
      logger.debug(`It's a warning button, continuing`);
      const buttons = await page.$$('button');
      if (buttons.length >= 3) await buttons[2].click();
      logger.debug(`Wait more 5 seconds for the consent summary page`);
      await sleep(5000);
    }

    // Approve the consent
    if (page.url().includes('consentsummary')) {
      logger.debug(`It's the consent summary page, clicking continue`);
      const buttons = await page.$$('button');
      if (buttons.length >= 3) await buttons[2].click();
    }

    logger.debug(`Waiting another 3 seconds to finish the auth.`);
    await sleep(3000);
